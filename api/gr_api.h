...
struct gr_api_request {
	uint32_t id; // Unique request identifier (auto-generated by client).
	uint32_t type; // Request type (REQUEST_TYPE macro result).
	uint32_t payload_len; // Payload length in bytes (max GR_API_MAX_MSG_LEN).
};

// API response header.
struct gr_api_response {
	uint32_t for_id; // Matches gr_api_request.id (for out-of-order handling).
	uint32_t status; // Status code (errno values, 0 for success).
	uint32_t payload_len; // Payload length in bytes (max GR_API_MAX_MSG_LEN).
};
...
struct gr_hello_req {
	char version[128]; // Client version string (NUL-terminated).
};
...
struct gr_event_subscribe_req {
	...
	uint32_t ev_type; // Event type to subscribe to (or EVENT_TYPE_ALL).
};
...
// API event header.
struct gr_api_event {
	uint32_t ev_type; // Event type (from EVENT_TYPE macro).
	size_t payload_len; // Event payload length (max GR_API_MAX_MSG_LEN).
};
