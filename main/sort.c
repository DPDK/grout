...
static bool *build_adjacency_matrix(arr const void **nodes, topo_is_child_cb_t is_child) {
	const unsigned len = arr_len(nodes);
...
int topo_sort(arr const void **nodes, topo_is_child_cb_t is_child) {
	if (is_child == NULL)
