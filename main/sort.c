...
static bool *build_adjacency_matrix(
	...
	for (unsigned i = 0; i < len; i++) {
		for (unsigned j = 0; j < len; j++) {
			if (i == j)
				continue;
			if (is_parent != NULL) {
				matrix[j * len + i] = is_parent(nodes[i], nodes[j]);
			} else {
...
int topo_sort(
	...
	if (is_parent == NULL && is_child == NULL)
	...
	matrix = build_adjacency_matrix(nodes, is_parent, is_child);
	if (matrix == NULL)
		goto out;
	...
	for (unsigned i = 0; i < len / 2; i++) {
		const void *tmp = sorted[i];
		sorted[i] = sorted[len - 1 - i];
		sorted[len - 1 - i] = tmp;
	}

	memcpy(nodes, sorted, len * sizeof(void *));
