...
enum zebra_dplane_result grout_add_del_route(struct zebra_dplane_ctx *ctx) {
	...
	if (dplane_ctx_get_vrf(ctx) != 0) {
		gr_log_err(
			"impossssible to add/del route on vrf %u (vrf not supported)",
	...
	if (p->family != AF_INET && p->family != AF_INET6) {
		gr_log_err(
			"impossssible to add/del route with family %u (not supported)", p->family
		);
		return ZEBRA_DPLANE_REQUEST_FAILURE;
	}
	...
	if (nt == NEXTHOP_TYPE_BLACKHOLE) {
		gr_log_err(
			"impossssible to add/del route with nexthope type = %u (not supported)", nt
		);
		return ZEBRA_DPLANE_REQUEST_FAILURE;
	}
	...
	if (p->family == AF_INET) {
		...
		if (nt == NEXTHOP_TYPE_IPV6 || nt == NEXTHOP_TYPE_IPV6_IFINDEX) {
			gr_log_err(
				"impossssible to add/del ipv4 route with nexthope type = %u (not "
				"supported)",
				nt
			);
			return ZEBRA_DPLANE_REQUEST_FAILURE;
		...
		if (nt == NEXTHOP_TYPE_IPV4 || nt == NEXTHOP_TYPE_IPV4_IFINDEX) {
			gr_log_err(
				"impossssible to add/del ipv6 route with nexthope type = %u (not "
				"supported)",
				nt
			);
			return ZEBRA_DPLANE_REQUEST_FAILURE;
		...
		if (new) {
			...
			if (nt == NEXTHOP_TYPE_IFINDEX || nt == NEXTHOP_TYPE_IPV6_IFINDEX)
				// TOFIX with next API in grout
				memcpy(req.r6_add.nh.a, p->u.prefix6.s6_addr, sizeof(req.r6_add.nh)
				);
			req.r4_add.nh = p->u.prefix4.s_addr;
			if (nt == NEXTHOP_TYPE_IPV6 || nt == NEXTHOP_TYPE_IPV6_IFINDEX)
