...
static void grout_neigh_notify(bool new, struct gr_nexthop *gr_nh) {
	...
	dplane_ctx_set_op(ctx, new ? DPLANE_OP_NEIGH_IP_INSTALL : DPLANE_OP_NEIGH_IP_DELETE);
...
void grout_nexthop_change(bool new, struct gr_nexthop *gr_nh, bool startup) {
	...
	if (!new) {
		zebra_nhg_kernel_del(gr_nh->nh_id, vrf_grout_to_frr(gr_nh->vrf_id));
		return;
	}
	...
	zebra_nhg_kernel_find(
		gr_nh->nh_id, nh, NULL, 0, vrf_grout_to_frr(gr_nh->vrf_id), afi, type, startup, NULL
	);
...
static void neigh_install_nexthop(struct zebra_dplane_ctx *ctx, const struct ipaddr *addr) {
	...
	l3 = (struct gr_nexthop_info_l3 *)req->nh.info;
	l3->flags = GR_NH_F_REMOTE | GR_NH_F_NEIGH;
