...
static ssize_t format_nexthop_info_group(char *buf, size_t len, const void *info) {
	...
	for (uint32_t i = 0; i < grp->count; i++)
		SAFE_BUF(snprintf, len, "id(%d/%d) ", grp->nh_ids[i], grp->weights[i]);
	return n;
err:
	return -errno;
}
...
static cmd_status_t nh_group_add(struct gr_api_client *c, const struct ec_pnode *p) {
	...
	while (n) {
		...
		if (arg_u32(n, "NHID", &group->nh_ids[group->count]) < 0)
			goto out;
		if (arg_u32(n, "WEIGHT", &group->weights[group->count]) < 0) {
			if (errno == ENOENT)
				group->weights[group->count] = 0;
			else
				goto out;
		}
