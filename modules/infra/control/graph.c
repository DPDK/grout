...
static arr const char **base_node_names;
static arr char **rx_node_names;
static arr char **tx_node_names;
...
static struct iface_info_port *find_port(arr struct iface_info_port **ports, uint16_t port_id) {
	struct iface_info_port *port = NULL;
	arr_foreach (struct iface_info_port *p, ports) {
...
static int
worker_graph_new(struct worker *worker, uint8_t index, arr struct iface_info_port **ports) {
	arr const char **graph_nodes = NULL;
	char graph_name[RTE_GRAPH_NAMESIZE];
	char node_name[RTE_NODE_NAMESIZE];
	arr char **rx_nodes = NULL;
	...
	arr_foreach_ref (qmap, worker->rxqs) {
		...
		arr_add(rx_nodes, name);
		arr_add(graph_nodes, name);
	}

	arr_extend(graph_nodes, base_node_names);
	arr_extend(graph_nodes, tx_node_names);

	// graph init
	struct rte_graph_param params = {
		.socket_id = rte_lcore_to_socket_id(worker->lcore_id),
		.nb_node_patterns = arr_len(graph_nodes),
	...
	arr_foreach_ref (qmap, worker->rxqs) {
		...
		burst_size = RTE_MAX(1u, graph_conf.vector_max / arr_len(rx_nodes));
	...
	arr_foreach (const char *name, tx_node_names) {
	...
	arr_foreach_ref (qmap, worker->txqs) {
		...
		STAILQ_FOREACH (w, &workers, next) {
			arr_foreach_ref (struct queue_map *q, w->txqs) {
		...
		for (rte_edge_t edge = 0; edge < arr_len(tx_node_names); edge++) {
...
out:
	arr_free(graph_nodes);
	strarr_free(rx_nodes);

	return errno_set(-ret);
}

int worker_graph_reload(struct worker *worker, arr struct iface_info_port **ports) {
...
static char *ensure_queue_node(
	...
	for (unsigned i = 0; i < arr_len(old_list); i++) {
...
static arr rte_node_t *worker_graph_nodes_add_missing(arr struct iface_info_port **ports) {
	arr char **old_rx = arr_clone(rx_node_names);
	arr char **old_tx = arr_clone(tx_node_names);
	rte_node_t *unused_nodes = NULL;
	rte_edge_t edge;
	char *name;

	arr_free(rx_node_names);
	arr_free(tx_node_names);

	// clone port_rx and port_tx to match all possible port-queue pairs
	arr_foreach (struct iface_info_port *port, ports) {
		for (uint16_t rxq = 0; rxq < port->n_rxq; rxq++) {
			name = ensure_queue_node(
				port_rx_node, RX_NODE_FMT, port->port_id, rxq, old_rx
			);
			arr_add(rx_node_names, name);
		}
		for (uint16_t txq = 0; txq < port->n_txq; txq++) {
			name = ensure_queue_node(
				port_tx_node, TX_NODE_FMT, port->port_id, txq, old_tx
			);
			arr_add(tx_node_names, name);
		}
	}

	// update the port_output edges to allow reaching all possible port_tx clones
	edge = rte_node_edge_update(
		port_output_node, 0, (const char **)tx_node_names, arr_len(tx_node_names)
	);
	assert(edge != RTE_EDGE_ID_INVALID);
	port_output_invalid = arr_len(tx_node_names);

	// this error edge must be last
	const char *invalid_port = "port_output_invalid";
	edge = rte_node_edge_update(port_output_node, arr_len(tx_node_names), &invalid_port, 1);
	assert(edge != RTE_EDGE_ID_INVALID);

	edge = rte_node_edge_shrink(port_output_node, arr_len(tx_node_names) + 1);
	assert(edge != RTE_EDGE_ID_INVALID);

	// store all unused node_ids in a list to be returned to the caller
	arr_foreach (name, old_rx)
		arr_add(unused_nodes, rte_node_from_name(name));
	arr_foreach (name, old_tx)
		arr_add(unused_nodes, rte_node_from_name(name));
	strarr_free(old_rx);
	strarr_free(old_tx);
...
int worker_graph_reload_all(arr struct iface_info_port **ports) {
	struct worker *worker;
	int ret;

	arr rte_node_t *unused_nodes = worker_graph_nodes_add_missing(ports);
	...
	arr_foreach (rte_node_t node, unused_nodes)
		rte_node_free(node);
	arr_free(unused_nodes);
...
static struct api_out graph_dump(const void *request, struct api_ctx *) {
	...
	arr const char **seen_edges = NULL;
	...
	STAILQ_FOREACH (info, &node_infos, next) {
		...
		for (unsigned i = 0; i < nb_edges; i++) {
			...
			arr_foreach (const char *e, seen_edges) {
				if (strcmp(e, edge) == 0)
					goto skip; // skip duplicate edges
			}
			arr_add(seen_edges, edge);
...
static void graph_init(struct event_base *) {
	...
	STAILQ_FOREACH (info, &node_infos, next) {
		...
		if (strcmp(reg->name, RX_NODE_BASE) == 0)
			...
			arr_add(base_node_names, reg->name);
...
static void graph_fini(struct event_base *) {
	...
	arr_free(base_node_names);
	strarr_free(rx_node_names);
	strarr_free(tx_node_names);
}
