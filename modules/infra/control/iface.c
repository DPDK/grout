...
struct iface *iface_create(const struct gr_iface *conf, const void *api_info) {
	...
	if (conf->master_id == GR_IFACE_ID_UNDEF) {
	...
	if (conf->master_id != GR_IFACE_ID_UNDEF) {
		...
		master = iface_from_id(conf->master_id);
		if (master == NULL)
			goto fail;

		t = iface_type_get(master->type);
		assert(t != NULL);

		if (t->attach_master == NULL) {
			errno = EMEDIUMTYPE;
			goto fail;
		}
		if (t->attach_master(master, iface) < 0)
			goto fail;
	} else {
...
static void detach_master(struct iface *iface) {
	...
	master = iface_from_id(iface->master_id);
	if (master == NULL)
		return;

	type = iface_type_get(master->type);
	assert(type != NULL);

	if (type->detach_master == NULL)
		iface->master_id = GR_IFACE_ID_UNDEF;
	else if (type->detach_master(master, iface) < 0)
		LOG(WARNING, "%s: detach from %s: %s", iface->name, master->name, strerror(errno));
}
...
int iface_reconfig(
	...
	if (set_attrs & GR_IFACE_SET_VRF) {
		...
		iface->mode = GR_IFACE_MODE_VRF;
	} else if (set_attrs & GR_IFACE_SET_MASTER) {
		struct iface *master = iface_from_id(conf->master_id);
		if (master == NULL)
			goto err;
		...
		if ((ret = type->attach_master(master, iface)) < 0)
			goto err;
	}
