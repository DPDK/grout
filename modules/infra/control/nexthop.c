...
struct nexthop *nexthop_new(const struct gr_nexthop *base) {
	...
	if (af_index >= 0 && nh->vrf_id < MAX_VRFS) {
		...
		atomic_fetch_add(&stats->by_state[GR_NH_S_NEW], 1);
	}
...
void nexthop_decref(struct nexthop *nh) {
	if (nh->ref_count <= 1) {
		...
		if (af_index >= 0 && nh->vrf_id < MAX_VRFS) {
			struct nh_stats *stats = &nh_stats[nh->vrf_id][af_index];
			if (nh->state < GR_NH_STATE_COUNT
			    && atomic_load(&stats->by_state[nh->state]) > 0)
				atomic_fetch_sub(&stats->by_state[nh->state], 1);
			if (atomic_load(&stats->total) > 0)
				atomic_fetch_sub(&stats->total, 1);
		}
...
void nh_stats_update(struct nexthop *nh, gr_nh_state_t new_state) {
	...
	if (nh->state != new_state) {
		...
		if (nh->state < GR_NH_STATE_COUNT && atomic_load(&stats->by_state[nh->state]) > 0)
			atomic_fetch_sub(&stats->by_state[nh->state], 1);

		// Increase new state count
		if (new_state < GR_NH_STATE_COUNT)
			atomic_fetch_add(&stats->by_state[new_state], 1);
	}
}
