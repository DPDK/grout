...
static int telemetry_datapath_stats_get(
	...
	for (uint16_t vrf_id = 0; vrf_id < MAX_VRFS; vrf_id++) {
		...
		if (rib4_stats)
			total_routes += rib4_stats->total_routes;
		if (rib6_stats)
			total_routes += rib6_stats->total_routes;
		if (total_routes == 0)
			continue;
		...
		if (rib4_stats) {
			...
			rte_tel_data_add_dict_uint(
				ipv4_data, "total_routes", rib4_stats->total_routes
			);
			...
			for (int i = 0; i <= GR_NH_ORIGIN_MAX; i++) {
				if (rib4_stats->by_origin[i] > 0) {
					rte_tel_data_add_dict_uint(
						origins_data,
						gr_nh_origin_name(i),
						rib4_stats->by_origin[i]
					);
				}
			}
		...
		if (rib6_stats) {
			...
			rte_tel_data_add_dict_uint(
				ipv6_data, "total_routes", rib6_stats->total_routes
			);
			...
			for (int i = 0; i <= GR_NH_ORIGIN_MAX; i++) {
				if (rib6_stats->by_origin[i] > 0) {
					rte_tel_data_add_dict_uint(
						origins_data,
						gr_nh_origin_name(i),
						rib6_stats->by_origin[i]
					);
				}
			}
