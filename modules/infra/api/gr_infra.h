...
typedef enum : uint8_t {
	...
	GR_IFACE_TYPE_PORT, // Physical or virtual DPDK port.
	GR_IFACE_TYPE_VLAN, // VLAN sub-interface.
	GR_IFACE_TYPE_IPIP, // IP-in-IP tunnel interface.
	GR_IFACE_TYPE_BOND, // Aggregation of DPDK ports.
	GR_IFACE_TYPE_COUNT
} gr_iface_type_t;

// Interface configuration flags.
typedef enum : uint16_t {
	GR_IFACE_F_UP = GR_BIT16(0), // Interface is administratively up.
...
typedef enum : uint16_t {
	GR_IFACE_S_RUNNING = GR_BIT16(0), // Interface is operationally running.
	GR_IFACE_S_PROMISC_FIXED = GR_BIT16(1), // Forced on when MAC filter is full.
	GR_IFACE_S_ALLMULTI = GR_BIT16(2), // Always on for ports.
} gr_iface_state_t;
...
typedef enum : uint8_t {
	GR_IFACE_MODE_L3 = 0, // Layer 3 routing mode.
	GR_IFACE_MODE_L1_XC, // Layer 1 cross-connect mode.
	GR_IFACE_MODE_COUNT
} gr_iface_mode_t;

// Interface reconfiguration attribute flags.
...
// Generic base structure for all network interfaces.
struct __gr_iface_base {
	...
	union {
		uint16_t vrf_id; // L3 routing domain.
		uint16_t domain_id; // L2 cross-connect peer interface ID.
	};
...
struct __gr_iface_info_port_base {
	...
	uint16_t rxq_size;
	uint16_t txq_size;
	uint16_t bond_iface_id; // Bond to which this port belongs (if any).
...
#define GR_PORT_DEVARGS_SIZE 64
	// e.g., "0000:01:00.0" or "net_virtio_user,path=/run/vhost/foo"
	char devargs[GR_PORT_DEVARGS_SIZE];
#define GR_PORT_DRIVER_NAME_SIZE 32
	// e.g., "net_ice" or "net_virtio"
...
struct gr_infra_iface_add_req {
	// Interface to add including type-specific info.
	// Name must be unique and valid UTF-8 (iface_id is ignored).
...
struct gr_iface_stats {
	...
	uint64_t rx_packets;
	uint64_t rx_bytes;
	uint64_t rx_drops; // Missed packets because hardware rxq full.
	uint64_t tx_packets;
	uint64_t tx_bytes;
	uint64_t tx_errors; // Transmission errors.
	// Control plane linux interface counters.
