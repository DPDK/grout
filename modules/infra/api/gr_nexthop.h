...
typedef enum : uint8_t {
	GR_NH_ORIGIN_UNSPEC = 0, // Unspecified origin.
	GR_NH_ORIGIN_REDIRECT = 1, // Installed by ICMP redirect.
	GR_NH_ORIGIN_LINK = 2, // For local addresses, no ID allocation (NH_ORIGIN_KERNEL).
	GR_NH_ORIGIN_BOOT = 3, // Installed at boot??
...
#define GR_NH_ID_UNSET UINT32_C(0) // Auto-allocate nexthop ID from pool.
...
struct gr_nexthop_group_member {
	...
	uint32_t weight; // Used for weighted multipath routing.
};
...
struct gr_nh_list_req {
	uint16_t vrf_id; // GR_VRF_ID_ALL for all VRFs.
	gr_nh_type_t type; // GR_NH_T_ALL for all types.
	bool include_internal; // Include GR_NH_ORIGIN_INTERNAL nexthops in results.
};
