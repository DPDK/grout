...
void gr_snap_input_add_mac_redirect(const struct rte_ether_addr *mac_addr, const char *next_node) {
	...
	if (!rte_is_multicast_ether_addr(mac_addr))
		ABORT("MAC redirect only allowed for multicast addresses: " ETH_F, mac_addr);
	if (rte_is_broadcast_ether_addr(mac_addr))
		ABORT("MAC redirect not allowed for broadcast addresses: " ETH_F, mac_addr);
...
snap_input_process(struct rte_graph *graph, struct rte_node *node, void **objs, uint16_t nb_objs) {
	...
	for (uint16_t i = 0; i < nb_objs; i++) {
		...
		for (uint16_t i = 0; i < mac_redirect_count; i++) {
			if (rte_is_same_ether_addr(&eth->dst_addr, &mac_redirects[i].mac_addr)) {
				edge = mac_redirects[i].edge;
				break;
			}
		}
