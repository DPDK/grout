...
static const struct iface *bond_select_tx_member(const struct iface_info_bond *bond) {
	switch (bond->mode) {
	case GR_BOND_MODE_ACTIVE_BACKUP:
		if (bond->active_member < ARRAY_DIM(bond->members))
			return bond->members[bond->active_member];
	}
