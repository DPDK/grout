...
static uint16_t iface_output_process(
	...
	uint16_t last_iface_id, iface_id, vlan_id, packets;
	...
	for (uint16_t i = 0; i < nb_objs; i++) {
		...
		iface_id = iface->id;
		stats = iface_get_stats(rte_lcore_id(), iface_id);
		...
		iface_inout_mbuf_data(m)->iface = iface;
		iface_inout_mbuf_data(m)->vlan_id = vlan_id;
