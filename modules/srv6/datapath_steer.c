...
srv6_steer_process(struct rte_graph *graph, struct rte_node *node, void **objs, uint16_t nb_objs) {
	...
	for (uint16_t i = 0; i < nb_objs; i++) {
		...
		if (sd->n_nh > 1) {
			srh = (struct ip6_sr_rthdr *)(outer_ip6 + 1);
			srh->nxt = proto;
			srh->len = (hdrlen - sizeof(*outer_ip6)) / 8 - 1;
			srh->type = IP6_ROUTING_HEADER_TYPE_SRV6;
			srh->segleft = sd->n_nh - 1;
			srh->last_entry = sd->n_nh - 1;
			srh->flags = 0;
			srh->tag = 0;
			for (k = 0; k < sd->n_nh; k++)
				srh->segments[sd->n_nh - k - 1] = sd->nh[k];
...
static struct rte_node_register srv6_steer_node = {
	...
	.next_nodes = {
		...
		[INVALID] = "srv6_pkt_invalid",
		[NO_ROUTE] = "srv6_steer_no_route",
...
GR_DROP_REGISTER(srv6_pkt_invalid);
GR_DROP_REGISTER(srv6_steer_no_route);
