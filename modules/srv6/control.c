...
static struct api_out srv6_steer_add(const void *request, void ** /*response*/) {
	...
	if (req->s.is_dest6) {
		...
		nh = nexthop_new(
			GR_NH_SR6_STEER_V6, req->s.vrf_id, GR_IFACE_ID_UNDEF, &req->s.dest6.ip
		);
		if (nh == NULL)
		...
		nh = nexthop_new(
			GR_NH_SR6_STEER_V4, req->s.vrf_id, GR_IFACE_ID_UNDEF, &req->s.dest4.ip
		);
		if (nh == NULL)
			return api_out(errno, 0);
...
static struct api_out srv6_steer_list(const void *request, void **response) {
	...
	for (i = 0; i < resp->n_steer; i++) {
		...
		if (nh->type == GR_NH_SR6_STEER_V6) {
			psd->is_dest6 = true;
			psd->dest6.ip = nh->ipv6;
			psd->dest6.prefixlen = nh->prefixlen;
		} else if (nh->type == GR_NH_SR6_STEER_V4) {
			psd->is_dest6 = false;
...
static struct api_out srv6_localsid_add(const void *request, void ** /*response*/) {
	...
	// match this entry for srv6 pkt with DA == localsid
