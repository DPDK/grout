...
struct conn *gr_conn_lookup(const struct conn_key *key, conn_flow_t *flow) {
	...
	if (rte_hash_lookup_data(atomic_load(&conn_hash), key, &data) < 0)
		return NULL;
...
struct conn *gr_conn_insert(const struct conn_key *fwd_key, const struct conn_key *rev_key) {
	struct rte_mempool *pool = atomic_load(&conn_pool);
	struct rte_hash *hash = atomic_load(&conn_hash);
	struct conn *conn;
	void *data;

	// create a new connection object
	if (rte_mempool_get(pool, &data) < 0)
		return NULL;
	...
	if (rte_hash_add_key_data(hash, fwd_key, conn_data(data, CONN_FLOW_FWD)) < 0) {
		// hash full
	...
	if (rte_hash_add_key_data(hash, rev_key, conn_data(data, CONN_FLOW_REV)) < 0) {
		// hash full, remove forward key,
		rte_hash_del_key(hash, fwd_key);
		rte_mempool_put(pool, data);
		return NULL;
	}
...
static void do_ageing(evutil_socket_t, short /*what*/, void * /*priv*/) {
	...
	gr_vec struct conn **expired_conns = NULL;
	...
	iter = 0;
	while (rte_hash_iterate(h, &key, &data, &iter) >= 0) {
		conn = conn_ptr(data);
		...
		if (age > timeout)
			gr_vec_add(expired_conns, conn);
	}
	gr_vec_foreach (conn, expired_conns)
...
static int config_update(const struct gr_conntrack_config *new_conf) {
	struct rte_hash *cur_h = atomic_load(&conn_hash);
	if ((new_conf->max_count != 0 && new_conf->max_count != conf.max_count)
	    || atomic_load(&conn_pool) == NULL || cur_h == NULL) {
		uint32_t iter = 0;
		const void *key;
		char name[128];
		void *data;

		if (cur_h != NULL && rte_hash_iterate(cur_h, &key, &data, &iter) >= 0)
		...
		rte_mempool_free(atomic_exchange(&conn_pool, p));
		rte_hash_free(atomic_exchange(&conn_hash, h));
...
static struct api_out conntrack_list(const void * /*request*/, void **response) {
	...
	struct gr_conntrack *conntracks = NULL;
	struct conn *conn;
	uint32_t next = 0;
	const void *key;
	size_t len;
