...
static rte_rwlock_t policies_lock = RTE_RWLOCK_INITIALIZER;
...
int snat44_dynamic_policy_add(const struct gr_snat44_policy *p) {
	...
	rte_rwlock_write_lock(&policies_lock);
	STAILQ_INSERT_TAIL(&policies, policy, next);
	rte_rwlock_write_unlock(&policies_lock);
