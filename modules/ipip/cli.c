...
static void ipip_show(const struct br_api_client *c, const struct br_iface *iface) {
	const struct br_iface_info_ipip *ipip = (const struct br_iface_info_ipip *)iface->info;
...
static void
ipip_list_info(const struct br_api_client *c, const struct br_iface *iface, char *buf, size_t len) {
	const struct br_iface_info_ipip *ipip = (const struct br_iface_info_ipip *)iface->info;
	char local[64], remote[64];
...
static uint64_t parse_ipip_args(
	...
	struct br_iface_info_ipip *ipip;
	const char *local, *remote;

	ipip = (struct br_iface_info_ipip *)iface->info;
...
static cmd_status_t ipip_add(const struct br_api_client *c, const struct ec_pnode *p) {
	const struct br_infra_iface_add_resp *resp;
	struct br_infra_iface_add_req req = {
		.iface = {.type = BR_IFACE_TYPE_IPIP, .flags = BR_IFACE_F_UP}
	};
	void *resp_ptr = NULL;
...
static cmd_status_t ipip_set(const struct br_api_client *c, const struct ec_pnode *p) {
...
static int ctx_init(struct ec_node *root) {
	...
	ret = CLI_COMMAND(
		...
		with_help(
			"Interface name.",
			ec_node_dyn("NAME", complete_iface_names, INT2PTR(BR_IFACE_TYPE_IPIP))
		),
