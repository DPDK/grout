...
l3_redirect_process(struct rte_graph *graph, struct rte_node *node, void **objs, uint16_t nb_objs) {
	...
	for (uint16_t i = 0; i < nb_objs; i++) {
		...
		if (mbuf->packet_type & RTE_PTYPE_L3_IPV4) {
			...
			dst = rte_be_to_cpu_32(d->dst);
			if (RTE_IS_IPV4_MCAST(ip)) {
				dst.addr_bytes[0] = 0x01;
				dst.addr_bytes[1] = 0x00;
				dst.addr_bytes[2] = 0x5e;
				dst.addr_bytes[3] = (dst >> 16) & 0x7f;
				dst.addr_bytes[4] = (dst >> 8) & 0xff;
				dst.addr_bytes[5] = dst & 0xff;
			}
			...
			if (rte_ipv6_addr_is_linklocal(&d->src)) {
				src.addr_bytes[0] = d->src.a[8] ^ 0x02;
