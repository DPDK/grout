	...
	if (iface->type == GR_IFACE_TYPE_VLAN) {
		...
		if (iface == NULL) {
			LOG(ERR,
			    ...
			    iface_info_vlan(iface)->parent_id);
			goto err;
		}
		...
		if (iface == NULL) {
			LOG(ERR, "Active member in bond %u is NULL", iface->id);
			goto err;
		}
	}
...
static void cp_create(struct iface *iface) {
	...
	snprintf(ifalias, IFALIASZ, "\"Grout Control Plane interface for '%s'\"", iface->name);
...
static void cp_update(struct iface *iface) {
	...
	if (iface->type == GR_IFACE_TYPE_PORT || iface->type == GR_IFACE_TYPE_VLAN) {
