...
ip_output_process(struct rte_graph *graph, struct rte_node *node, void **objs, uint16_t nb_objs) {
	...
	for (i = 0; i < nb_objs; i++) {
		...
		if (iface->flags & GR_IFACE_F_SNAT) {
			const struct nexthop *nat = fib4_lookup(iface->vrf_id, ip->src_addr);

			if (nat != NULL && (nat->flags & GR_NH_F_SNAT)) {
				ip->hdr_checksum = fixup_checksum(
					ip->hdr_checksum, ip->src_addr, nat->priv.ipv4
				);
				ip->src_addr = nat->priv.ipv4;
			}
		}
