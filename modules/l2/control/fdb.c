...
static int fdb_reconfig(unsigned max_entries) {
	...
	struct rte_hash_parameters params = {
		...
		.key_len = sizeof(struct gr_fdb_key),
		.entries = max_entries,
	};
...
static struct api_out fdb_add(const void *request, struct api_ctx *) {
	...
	struct gr_fdb_entry fdb = req->fdb;
	...
	fdb.bridge_id = iface->id;
	fdb.last_seen = gr_clock_us();

	if (rte_hash_lookup_data(fdb_hash, &fdb.key, &data) < 0) {
		...
		*e = fdb;
		...
		if ((ret = rte_hash_add_key_data(fdb_hash, &fdb.key, data)) < 0) {
...
static struct api_out fdb_del(const void *request, struct api_ctx *) {
	...
	ret = rte_hash_del_key(fdb_hash, &req->key);
...
static struct api_out fdb_flush(const void *request, struct api_ctx *) {
	...
	while (rte_hash_iterate(fdb_hash, &key, &data, &next) >= 0) {
		...
		if (!(req->flags & GR_FDB_F_STATIC) && (fdb->flags & GR_FDB_F_STATIC))
			continue;
		if (req->bridge_id != GR_IFACE_ID_UNDEF && fdb->bridge_id != req->bridge_id)
			continue;
		if (req->iface_id != GR_IFACE_ID_UNDEF && fdb->iface_id != req->iface_id)
			continue;
		if (!rte_is_zero_ether_addr(&req->mac)
		    && !rte_is_same_ether_addr(&fdb->mac, &req->mac))
			continue;
