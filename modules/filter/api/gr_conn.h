...
struct gr_conn {
	...
	addr_family_t af;
	uint8_t proto;
	struct gr_conn_flow fwd_flow;
	struct gr_conn_flow rev_flow;
	gr_conn_state_t state;
	uint32_t id;
	clock_t last_update;
};

#define GR_CONN_MODULE 0xc0c0

// conntrack list //////////////////////////////////////////////////////////////

#define GR_CONN_LIST REQUEST_TYPE(GR_CONN_MODULE, 0x0001)

// struct gr_conn_list_req { };

struct gr_conn_list_resp {
	uint32_t n_conns;
	struct gr_conn conns[/* n_conns */];
};
...
#define GR_CONNTRACK_CONF_GET REQUEST_TYPE(GR_CONN_MODULE, 0x0002)
...
#define GR_CONNTRACK_CONF_SET REQUEST_TYPE(GR_CONN_MODULE, 0x0003)
