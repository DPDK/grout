...
#define CONN_S_IDENT 255 // special value for retransmissions
...
static const gr_conn_state_t tcp_state_machine[CONN_S_COUNT][CONN_FLOW_COUNT][TCP_FS_COUNT] = {
	...
	[CONN_S_SYN_RECEIVED] = {
		...
		[CONN_FLOW_REV] = {
			...
			[TCP_FS_SYNACK] = CONN_S_IDENT,
			// XXX: ACK of late SYN in simultaneous case?
			[TCP_FS_ACK] = CONN_S_IDENT,
			// FIN may occur early.
			[TCP_FS_FIN] = CONN_S_FIN_RECEIVED,
		},
	},
...
void gr_conn_update(struct conn *c, conn_flow_t flow, const struct rte_tcp_hdr *tcp) {
	gr_conn_state_t cur_state = atomic_load_explicit(&c->state, memory_order_relaxed);
	gr_conn_state_t new_state;
	...
	if (new_state != cur_state && new_state != CONN_S_IDENT)
		atomic_store_explicit(&c->state, new_state, memory_order_relaxed);

	atomic_store_explicit(&c->last_update, gr_clock_us(), memory_order_relaxed);
}
...
bool gr_conn_parse_key(
	...
	key->src.ipv4 = ip->src_addr;
	key->dst.ipv4 = ip->dst_addr;
...
static struct rte_mempool *conn_pool;
...
struct conn *gr_conn_insert(const struct conn_key *fwd_key, const struct conn_key *rev_key) {
	...
	if (rte_hash_add_key_data(conn_hash, rev_key, conn_data(data, CONN_FLOW_REV)) < 0) {
		...
		// conn_free will put the conntrack back in the pool
...
static void do_ageing(evutil_socket_t, short /*what*/, void * /*priv*/) {
	...
	while (rte_hash_iterate(conn_hash, &key, &data, &iter) >= 0) {
		...
		age = (now - atomic_load_explicit(&conn->last_update, memory_order_relaxed))
			/ 1000000ULL;
		if (age > timeout)
...
static void conn_free(void * /*priv*/, void *data) {
...
static int config_update(const struct gr_conntrack_config *new_conf) {
	if ((new_conf->max_count != 0 && new_conf->max_count != conf.max_count)
		...
		struct rte_mempool *old_p = conn_pool;
		struct rte_hash *old_h = conn_hash;
		conn_pool = p;
		conn_hash = h;
		rte_mempool_free(old_p);
		rte_hash_free(old_h);
	}
...
static struct api_out conn_list(const void * /*request*/, void **response) {
	struct gr_conn_list_resp *resp = NULL;
	struct gr_conn *connections = NULL;
	struct conn *conn;
	uint32_t next = 0;
	const void *key;
	size_t len;
	...
	while (rte_hash_iterate(conn_hash, &key, &data, &next) >= 0) {
		...
		conn = conn_ptr(data);
		struct gr_conn c = {
			.id = (void *)conn - conn_pool->pool_data,
			.iface_id = conn->fwd_key.iface_id,
			.af = conn->fwd_key.af,
			.proto = conn->fwd_key.proto,
			.fwd_flow = {
				.src.ipv4 = conn->fwd_key.src.ipv4,
				.dst.ipv4 = conn->fwd_key.dst.ipv4,
				.src_id = conn->fwd_key.src_id,
				.dst_id = conn->fwd_key.dst_id,
			},
			.rev_flow = {
				.src.ipv4 = conn->rev_key.src.ipv4,
				.dst.ipv4 = conn->rev_key.dst.ipv4,
				.src_id = conn->rev_key.src_id,
				.dst_id = conn->rev_key.dst_id,
			},
			.state = atomic_load_explicit(&conn->state, memory_order_relaxed),
			.last_update = atomic_load_explicit(&conn->last_update, memory_order_relaxed),
		};
		gr_vec_add(connections, c);
	}

	len = sizeof(*resp) + gr_vec_len(connections) * sizeof(*connections);
	if ((resp = calloc(1, len)) == NULL) {
		gr_vec_free(connections);
		return api_out(ENOMEM, 0);
	}

	resp->n_conns = gr_vec_len(connections);
	if (connections != NULL)
		memcpy(resp->conns, connections, resp->n_conns * sizeof(resp->conns[0]));
	gr_vec_free(connections);
	*response = resp;
