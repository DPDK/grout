...
static cmd_status_t conn_list(const struct gr_api_client *c, const struct ec_pnode *) {
	const struct gr_conn_list_resp *resp;
	...
	if (gr_api_client_send_recv(c, GR_CONN_LIST, 0, NULL, &resp_ptr) < 0)
		goto end;
	...
	for (size_t i = 0; i < resp->n_conns; i++) {
		...
		const struct gr_conn *conn = &resp->conns[i];
...
static int ctx_init(struct ec_node *root) {
	...
	ret = CLI_COMMAND(
		FILTER_SHOW_CTX(root), "conntrack", conn_list, "Display tracked connections."
	);
	if (ret < 0)
		return ret;
