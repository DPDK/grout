...
nat_verdict_t snat44_dynamic_process(const struct iface *iface, struct rte_mbuf *m) {
	...
	if (conn == NULL) {
		// No existing conntrack object.
		if (snat44_dynamic_policy_lookup(&fwd_key) == NULL) {
			// No SNAT rule configured for this source address.
			// Should we let packet pass?
			return NAT_VERDICT_DROP;
		}
		// Send packet to control plane for conntrack creation.
		return NAT_VERDICT_CONTROL;
	}

	// Existing conntrack found, perform source NAT.
	nat = &conn->nat;
