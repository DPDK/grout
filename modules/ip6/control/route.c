...
static inline uint32_t fib6_get_max_routes(const struct iface *vrf) {
	return iface_info_vrf(vrf)->fib[GR_VRF_FIB_IP6].max_routes;
}

static inline uint32_t fib6_get_num_tbl8(const struct iface *vrf) {
	return iface_info_vrf(vrf)->fib[GR_VRF_FIB_IP6].num_tbl8;
}
...
static struct rte_fib6 *get_fib6(uint16_t vrf_id) {
	...
	struct rte_fib6 *fib = vrf_fib_ptr_get(iface, GR_VRF_FIB_IP6);
	if (fib == NULL)
...
static int fib6_init(struct iface *vrf) {
	struct gr_iface_info_vrf_fib *conf = &iface_info_vrf(vrf)->fib[GR_VRF_FIB_IP6];
	struct rte_fib6 *fib;
	...
	vrf_fib_ptr_set(vrf, GR_VRF_FIB_IP6, fib);
	return 0;
}
...
void rib6_iter(uint16_t vrf_id, rib6_iter_cb_t cb, void *priv) {
	for (uint16_t v = 0; v < GR_MAX_IFACES; v++) {
		...
		if (fib == NULL)
			continue;
		rib6_iter_one(rte_fib6_get_rib(fib), v, cb, priv);
	}
}
...
static void rib6_metrics_collect(struct gr_metrics_writer *w) {
	...
	for (uint16_t vrf_id = 0; vrf_id < GR_MAX_IFACES; vrf_id++) {
		...
		struct rte_fib6 *fib = vrf_fib_ptr_get(vrf_iface, GR_VRF_FIB_IP6);
		if (fib != NULL) {
...
static int fib6_reconfig(struct iface *vrf) {
	...
	old_fib = vrf_fib_ptr_get(vrf, GR_VRF_FIB_IP6);
	new_fib = create_fib6(vrf);
	if (new_fib == NULL)
		return errno_log(errno, "create_fib6");

	struct fib6_migrate_ctx ctx = {.new_fib = new_fib};
	rib6_iter_one(rte_fib6_get_rib(old_fib), vrf->id, fib6_migrate_cb, &ctx);

	vrf_fib_ptr_set(vrf, GR_VRF_FIB_IP6, new_fib);
...
#ifdef HAVE_RTE_FIB_TBL8_GET_STATS
		struct rte_fib6 *fib = vrf_fib_ptr_get(vrf, GR_VRF_FIB_IP6);
		if (fib != NULL)
...
static void fib6_fini(struct iface *vrf) {
	...
	vrf_fib_ptr_set(vrf, GR_VRF_FIB_IP6, NULL);
	if (fib != NULL) {
		LOG(INFO, "destroying IPv6 FIB for VRF %s(%u)", vrf->name, vrf->id);
		rib6_iter_one(rte_fib6_get_rib(fib), vrf->id, route6_vrf_rm, (void *)vrf);
		rte_fib6_free(fib);
	}
