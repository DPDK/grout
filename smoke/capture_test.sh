...
timeout 10 grcli capture iface p0 count 10 > "$cap" &
cap_pid=$!
wait_event -t10 "capture start"
...
timeout 10 grcli capture any count 10 > "$cap_all" &
cap_pid=$!
wait_event -t10 "capture start"
...
timeout 10 grcli capture iface p0 count 5 > "$cap_reuse" &
cap_pid=$!
wait_event -t10 "capture start"
...
timeout 10 grcli capture iface p0 count 5 > "$cap_first" &
cap_pid_first=$!
timeout 10 grcli capture iface p1 count 5 > "$cap_second" &
cap_pid_second=$!
wait_event -t10 -c2 "capture start"
...
mark_events
timeout 10 grcli capture iface p0 >/dev/null &
cap_pid=$!
wait_event -t10 "capture start"

! grcli capture iface p0 count 1 >/dev/null ||
...
timeout 10 grcli capture iface p0 count 5 snaplen 64 > "$cap_snap" &
cap_pid=$!
wait_event -t10 "capture start"
...
timeout 10 grcli capture iface p0 count 5 filter icmp > "$cap_filter" &
cap_pid=$!
wait_event -t10 "capture start"
...
timeout 10 tcpdump -i grout:p0 -w "$cap_native" -c5 &
td_pid=$!
wait_event -t10 "capture start"
...
timeout 10 tcpdump -i grout:any -w "$cap_native_all" -c5 &
td_pid=$!
wait_event -t10 "capture start"
...
timeout 10 tcpdump -i grout:p0 -w "$cap_bpf" 'icmp' -c5 &
td_pid=$!
wait_event -t10 "capture start"
