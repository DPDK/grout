...
for n in 0 1 2; do
	...
	grcli fdb show iface p$n learn json | jq -e --arg mac "$mac" '.fdb[] | select(.mac == $mac)' > /dev/null
done
...
grcli fdb show iface p0 static json | jq -e --arg mac "$mac" '.fdb[] | select(.mac == $mac)' > /dev/null
...
if [ "$(grcli fdb show iface p1 json | jq '.fdb | length')" -ne 0 ]; then
	fail "fdb still contains entries for removed interface"
fi

grcli interface del br0
if [ "$(grcli fdb show json | jq '.fdb | length')" -ne 0 ]; then
