...
grcli interface add bridge br100 no_learn_vtep
...
ip -n evpn-peer link add br100 type bridge vlan_filtering 1
...
mac=$(ip netns exec host-a cat /sys/class/net/x-p1/address)
...
# -- Verify EVPN type-2 (MAC/IP) learned
grcli fdb show | grep "$mac.*172.16.0.1.*remote"
