...
# Create a port for underlay connectivity
port_add p0

# Configure underlay IP address
grcli address add 10.0.0.1/24 iface p0

# Create bridge with VXLAN member
...
# Add an IP address on the bridge for L3 connectivity
grcli address add 192.168.100.1/24 iface br100

# Show VXLAN interface configuration
grcli interface show name vxlan100

# Setup peer in netns with Linux VXLAN
netns_add n1
move_to_netns x-p0 n1
ip -n n1 addr add 10.0.0.2/24 dev x-p0

# Create Linux VXLAN interface
...
ip netns exec n1 ping -i0.01 -c3 -W1 192.168.100.1 || true
