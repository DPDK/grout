...
#
# Test SRv6 adjacency SIDs generated by ISIS.
#
# No SID is configured anywhere: ISIS allocates one per adjacency from the
# locator and binds it to the neighbour learnt from the hellos. The locator
# uses uSID, so what zebra installs is a uA rather than a plain End.X.
#
# network layout:
#  grout (ISIS + SRv6)                    isis-peer (ISIS + SRv6)
#       p0                                        x-p0
#  2001:db8:1::1/64 <------ ISIS adjacency ------> 2001:db8:1::2/64
#                       (adjacency SID auto-generated)
#
# IPv6 address plan:
...
if ! [ "$min_version" = "10.6.0" ]; then
       echo "$0: FRR $frr_version does not support SRv6 adjacency SIDs"
       exit 125
fi
