...
- `<test>.grcli` - grout configuration, applied on top of a partially configured
  daemon with `grcli -xef ndr/<test>.grcli`.
...
`ndr/*.grcli` and reading the name from either file of the pair.
...
sudo ndr/harness/validate.sh ipv4    # one scenario
make ndr-validate                    # validate all scenarios
```
...
5. Validate the grout side against the daemon: `grcli -xef ndr/<test>.grcli`
