...
#
# name: SRv6 L3VPN, reduced encap (H.Encaps.Red / End.DT4)
#
# Asymmetric SRv6 L3VPN profile using the reduced encapsulation (no SRH):
#   direction 0 (TRex port 0 -> grout p0): plain IPv4 packet toward the remote
#     site; grout H.Encaps.Red-encapsulates it (outer IPv6, no SRH) out p1;
#   direction 1 (TRex port 1 -> grout p1): reduced SRv6 packet (outer IPv6, no
#     SRH) destined to grout's local End.DT4 SID with an inner IPv4 packet; grout
#     decapsulates and routes the inner IPv4 out p0.
#
# Ingress direction 0 is a 64-byte IPv4 frame; ingress direction 1 is
# Ether(14) + IPv6(40) + IPv4(20) + UDP(8) = 82 bytes (no SRH).
...
class Profile:
    def get_streams(self, direction=0, tunables=(), **kwargs):
        if direction == 0:
            ...
            # Reduced SRv6 (no SRH): outer IPv6 dst is the SID, inner is IPv4.
